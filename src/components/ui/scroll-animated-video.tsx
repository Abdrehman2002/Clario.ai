...
export type HeroScrollVideoProps = {
  ...
  // Optional theme hint (not used yet, but kept for API stability)
...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        // overlayDarkenEl is closed over from the async function
      } catch {}
      try {
  ...
  return (
    ...
    >
      ...
      {/* Styles (scoped) - Updated to match purple theme */}
      <style>{`
        .hsv-root {
          --bg: #030303;
          --text: #f9fafb;
          --muted: #9ca3af;
          --muted-bg: rgba(17,24,39,0.8);
          --muted-border: rgba(148,163,184,0.35);
          --overlay-bg: rgba(3,4,7,0.4);
          --overlay-text: #ffffff;
          --accent: #9333ea;
          --accent-2: #7c3aed;
        ...
        .hsv-title {
          ...
          text-wrap: balance;
          background: linear-gradient(110deg, #ffffff 0%, #e5e7eb 35%, #9333ea 85%);
        ...
        .hsv-overlay-content h3 {
          ...
          background: linear-gradient(100deg, #ffffff 0%, #c4b5fd 30%, #9333ea 100%);
          -webkit-background-clip: text;
          background-clip: text;
          color: transparent;
          text-wrap: balance;
